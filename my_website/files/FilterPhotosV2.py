...
def frame_image(original_image, wide=15):
    ...
    #Looks for a folder named "Results V2" and makes it if it's not there
    directory = os.getcwd()
    new_directory = os.path.join(directory, 'Results V2')
    try:
    ...
    drawing_layer2.rectangle([(width/wide,height/wide), ((width/wide)*(wide-1),(height/wide)*(wide-1))],fill=(14,26,64,225))
    result = PIL.Image.new('RGBA', original_image.size, (14,26,64,123))
    result.paste(original_image,(0,0),mask=frame)
    
    #Adds the crest over the frame
    crest_img = PIL.Image.open('Royal_Crest.png')
...
def frame_all_images(directory=None):
    ...
    for n in range(len(image_list)):
        ...
        new_image_filename = os.path.join(new_directory, filename + '_framed_V2' + '.png')
